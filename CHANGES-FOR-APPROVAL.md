...
> |---|---|
> | `Our mission` — **3** items *(was 4 — D-102)* | `Notre mission` *(NC-20)* |
> | `Our commitments` — **3** items *(was 4 — D-103)* | `Nos engagements` *(NC-21)* |
...
>
> > ⚠ **Two of those translated sentences came back out on 2026-08-03**, on your
> > word, because putting `Our mission` and `Our commitments` under your live
> > `Our purpose` block made one page state the same commitment three times
> > (`D-83`). **Both blocks still ship; each now carries three items instead of
> > four.** The two that went were **ours, not yours** — `D-100` … `D-103`, and
> > your French page still carries all eight. The eight sections stand.
>
...
| **`D-`** | **this file only** | Build decision — a place the rebuild had to choose, where the choice is the client's to confirm | `D-01` … `D-32`, then `D-50` … `D-76`, then `D-90` … `D-97` *(the gaps are deliberate — parallel agents, and a gap is cheaper than a collision)* |
| **`NC-`** | **this file only** | New copy written for the rebuild | `NC-01` … `NC-07` |
| **`S-`** | **this file only** | Syndicated National Bank content | `S-01` … `S-08`, then `S-20` … *(the gap is deliberate — parallel page passes were each given their own block so two agents could not mint the same number. Later blocks may already be in use; this cell records the blocks that exist, not a ceiling.)* |
...
**One word in this table has been corrected, and you authorised it.** Everything
else is a live-site defect we found and **shipped exactly as it is**, because
`content/COMPLIANCE.md` §6 authorises no other editorial fix. They are listed so
you can approve or decline the rest in one pass. Declining costs nothing — the
site already says what your site says today.

> ⚠ **Changed 2026-08-03.** This paragraph used to open *"Not one word in this
> table has been corrected."* **`F-30` is now applied** — the twenty-four
> characters `Planification fiscale : ` are deleted from the front of one French
> paragraph, on your word. It ships as **`F-60`**, and the authorisation is
> written out, dated and scope-limited at `content/COMPLIANCE.md` **§6.6**.
> **Approving one row did not approve the set:** every other `F-` row below is
> still PENDING and still ships sic — **including `F-03`, which is in the same
> sentence.**

**Two rows are exceptions, and each is one for a precise reason.** `F-14` is
*applied* and changes **no string** — it exchanges two image files whose
descriptions were already right and already crossed. §6 governs wording, so a
change that alters no wording is not gated by an amendment; it is written out in
full under the table, and `content/COMPLIANCE.md` §6.4 and §7 check 49 both
record it. `F-60` is *applied* and **does** change a string — which is exactly
why it needed §6.6 and did not ship until you granted it.
...
| F-21 | Accueil (FR) | Live order: `<p>…vous pourrez toujours compter sur:</p>` → `<h3>Faisons connaissance</h3>` (a process step) | Same paragraph, unchanged, now followed by the client's own `Nos engagements` list — `Agir de manière intègre…`, `Demeurer informés…`, `Investir de manière audacieuse…`, `Respecter tous nos engagements…` — then the four process steps | **Applied — client decision 2026-08-03, pending NBF sign-off.** Supersedes F-20. **No word added, removed or altered.** The colon promises a list; the list exists on their own home page and has been moved here from its own section, where it no longer appears (it is not duplicated — verified once in `<main>`). Heading outline verified: no level skips. **English is unaffected** — its process block has no lead-in, so the FR and EN home pages now differ structurally by our hand rather than the client's; that asymmetry is ours and is flagged here. |
| F-30 | **Services**, **FR** — `Planification fiscale` | Live order: `<h2>Planification fiscale</h2>` → `<p>Planification fiscale : La planification fiscale est un processus…</p>` | ⚠ **NOW APPLIED — see `F-60`** *(was: unchanged (sic))* | ✅ **AUTHORISED AND APPLIED 2026-08-03.** This row is the recommendation; **`F-60` is what shipped**, and the authorisation is `content/COMPLIANCE.md` §6.6. The reasoning below is preserved unedited as the record of why it was recommended. ⚠ **`F-03` in the same sentence was NOT authorised and is still PENDING.** *(Original text follows.)* Shipped sic — **recommended, not applied.** The paragraph opens by repeating the heading directly above it, so the words `planification fiscale` land **three times inside twelve**: once as the heading, once as the label, once as the sentence's own subject. It is the only one of your seven French service blocks that does this — the other six open straight into their sentence (`La gestion de portefeuille consiste à…`, `La planification successorale est le processus…`). **The fix is a deletion and nothing else:** remove the 24 characters `Planification fiscale : ` from the front of the paragraph, leaving `La planification fiscale est un processus qui consiste à…`. **No word is rewritten and none is invented** — the label survives, unchanged, as the heading it already is. Not applied, because `content/COMPLIANCE.md` §6 still carries no editorial-fix amendment (§6.5 ¶6 confirms it, dated 2026-08-03) and a deletion from your copy is exactly what that gate governs. Not previously catalogued; found on this page. ⚠ The same paragraph also carries **F-03** (`à l’intérieure`); approving both is one edit. |
| F-31 | **Services**, **EN** — `Banking solutions` | Live order: `<h2>Banking solutions</h2>` → the PDF link `Wealth management - Banking-offer \| Exclusive benefits` → `<p>As a National Bank Financial—Wealth Management client, you can take care of all your banking in one place:</p>` → the two lists | **unchanged (sic)** | Shipped sic — **recommended, not applied.** A bare document link is the **first thing under the heading**, before the sentence that says what the section is about. A reader meets an offer document before being told there is an offer. Every other service block on that page opens on its own words. **The fix moves the link and changes nothing else:** put it after the two lists, where it reads as *"and here is the offer"* instead of arriving unannounced. **No word added, removed or altered** — only the order of two elements you already publish. Not applied, because reordering your own page is the same class of change as `F-21` and `F-21` needed your decision before it shipped. Not previously catalogued; found on this page. |
| F-50 | **Notre firme / Our Firm**, **both languages** — the join between your new paragraph and National Bank's | Your paragraph says `Notre mission est de sécuriser votre avenir financier…` / `Our mission is to secure your financial future…`. **The very next section says `La mission de la FBNGP est d'accompagner nos clients…` / `NBFWM's mission is to help our clients…`** — and the line under the page title already says the page is about Financière Banque Nationale – Gestion de patrimoine. A reader meets two different *missions*, one after the other, both apparently for the same firm. | **unchanged** | Shipped as built — **reported, not repaired.** There is no fix here that does not involve writing a sentence, and writing one is exactly what this build does not do. **Three ways out, all yours:** (a) leave it — the two missions are genuinely different, yours and the network's, and most readers will take it that way; (b) drop the mission sentence from `NC-27` / `NC-28`, which costs you your strongest sentence; (c) ask NBF whether the network paragraph may carry a heading that names FBNGP, which would separate the two voices without touching a word of either. **Our recommendation is (a)**, recorded so the decision is deliberate rather than accidental. |
| F-51 | **Our Firm**, **EN only** — three `partner` claims in a row | Your paragraph ends `…to be the main partner in your financial success.` The next heading reads `A financial partner for Canadian families`. The heading after that reads `A strong partner`. **Three consecutive blocks, three `partner` claims**, the last of which is four statistics with no sentence at all. | **unchanged** | Shipped as built — **reported, not repaired.** Two of the three are National Bank's own headings and the third is your sentence. **The French page has no such run** (`Le partenaire financier des familles canadiennes`, then `Un puissant allié`), so this is an English-only collision created by the English headings, not by your paragraph. Fixing it means rewording something, and every candidate belongs to somebody else. Worth one line to NBF. |
| F-52 | **Notre firme / Our Firm**, **both languages** — who `nous` / `we` means, and when it changes | Your paragraph opens `Notre équipe réunit Laurent Achard et David Alexandre Wolf, conseillers en gestion de patrimoine à Montréal` / `Our team brings together … wealth advisors in Montreal`. Two paragraphs later, in National Bank's copy: `**Notre** réseau compte près de 850 **conseillers en gestion de patrimoine** dans 100 succursales` / `**Our** network includes over 850 **wealth advisors** at 100 branches`. **Same pronoun, same job title, and the referent has silently gone from two people to a national network.** | `…Laurent Achard et David Alexandre Wolf, à Montréal.` / `…in Montreal.` | **Applied 2026-08-03.** **Applied — cut from our own authored copy, not the client's.** The identical phrase meant two people and then 850, so `nous`/`we` silently changed referent mid-page. **This defect did not exist before we added NC-27/NC-28** — it is the one place our addition changed how National Bank's copy reads. Verified: each phrase now appears once per page. Their titles are unaffected and still appear on the team page and both profile pages. <br>Reported and fixed under one id: this row was opened by the coherence pass and closed by the fix. |
| F-53 | **Notre firme / Our Firm**, **both languages** — `Contactez-nous` / `Contact us` sits one level too deep | Live and rebuilt, that block's heading is an `<h3>`. Everything above it on the page is an `<h2>`, the nearest of which is `À propos de la Banque Nationale` / `About National Bank of Canada`. **To a screen reader, your two direct phone numbers and both email addresses are announced as a subsection of "About National Bank of Canada".** Visually it looks like a top-level section, because it is styled as one. | **unchanged** | Shipped as built — **reported, not repaired, and blocked from our side.** The heading level is set in a shared component used by more than one page, and page work on this build is not allowed to edit shared components. **One character fixes it** (`h3` → `h2`) and the page outline stays valid — we checked. It needs a decision taken once, for every page that block appears on, by whoever owns the shared components. ⚠ Related, same class, same owner: **every English page carries two `<h2>Contact us</h2>` headings** — the closing call-to-action and the sticky contact strip both use that exact wording. The French pages read `Contactez-nous` and `Nous joindre`, which are different, so French is unaffected. That one is in the shared English dictionary. |
| **F-60** | **Services**, **FR** — `Planification fiscale` *(this is `F-30`, applied)* | `Planification fiscale : La planification fiscale est un processus qui consiste à définir une stratégie dont les objectifs consistent à réduire autant que possible la charge d’impôt et/ou d’en retarder le paiement en restant toujours à l’intérieure des dispositions de la loi.` | `La planification fiscale est un processus qui consiste à définir une stratégie dont les objectifs consistent à réduire autant que possible la charge d’impôt et/ou d’en retarder le paiement en restant toujours à l’intérieure des dispositions de la loi.` | **Applied — client decision 2026-08-03** (`content/COMPLIANCE.md` §6.6). **A deletion of exactly 24 characters and nothing else:** `Planification`, space, `fiscale`, space, `:`, space — all ordinary spaces, checked byte by byte against your live page (there is **no** non-breaking space before the colon, despite the French convention). **No word is rewritten, reordered, added or removed.** The heading `Planification fiscale` is unchanged and still sits directly above the paragraph, so the label is not lost — it is where it already was. Proof that nothing but the prefix moved: the shortened sentence is **still a character-for-character substring of your live page**, and our verbatim check on this file reports the same 24/26 match it did before the edit. ⚠ **`F-03` is in this very sentence and is untouched** — `à l’intérieure` still ships sic, one clause later, because you authorised this row and not that one. **Reversible by re-typing 24 characters in one file.** |
| ~~F-20~~ | Accueil (FR) | `Nos conseils, notre accompagnement et notre dévouement sont très appréciés de tous nos clients. En effet, vous pourrez toujours compter sur:` | `Nos conseils, notre accompagnement et notre dévouement sont très appréciés de tous nos clients.` | **SUPERSEDED by F-21 (client decision, 2026-08-03) — the colon is restored instead.** Deletion only; no word rewritten, none invented. The live colon promises a list that does not follow: the live document order is `<h2> Un processus éprouvé` → this `<p>` → `<h3> Faisons connaissance`, a process step rather than something a client can count on. Verified in `source-assets/html/fr-home.html`. **The client's own `Nos engagements` list is four things a client can count on** — if they prefer the colon kept, that list is the honest continuation and we would move it here. English is unaffected: its process block has no lead-in. |
...
>
> > ⚠ **REVISED 2026-08-03, and only in one direction: two of our own sentences
> > were CUT.** `NC-20` and `NC-21` shipped four items each. **They now ship
> > three each.** Putting `Our mission` and `Our commitments` directly under your
> > live `Our purpose` block made your English home page state the same
> > commitment three times over (`D-83`); the two items that were saying it again
> > are removed. **Every cut is from our translation. Not one character of your
> > own copy was touched, and your French page still carries all eight items.**
> > `D-100` … `D-103` record it, and each row below quotes what went and what it
> > duplicated. **A cut is not a rewrite:** the amendment authorises a faithful
> > rendering of your French or nothing at all, so a removed item stays removed
> > rather than coming back in different words.
>
...
|---|---|---|---|---|
| NC-20 | English home page — a new `Our mission` block | **⚠ THREE ITEMS AS SHIPPED, revised 2026-08-03 (D-100 / D-102).** `Our mission` · `Secure your financial future` · `Support you in achieving your ambitious life projects` · `Build a long-term relationship based on trust and professionalism`.<br>**Removed before shipping:** ~~`Be the main partner in your financial success`~~ — see the status column | `Notre mission` · `Sécuriser votre avenir financier ` · `Vous accompagner dans la réalisation de vos projets de vie ambitieux` · ~~`Être le partenaire principal dans votre réussite financière `~~ *(not shipped in English; **still live and unchanged on your French page**)* · `Construire une relation à long terme basée sur la confiance et le professionnalisme` *(French home page, C-26)* | ⚠ needs explicit approval — **NBF sign-off required.**<br>**What we cut, and what it duplicated.** `Be the main partner in your financial success` is gone. Your own live English block `Our purpose`, one section above this list, opens **`As your partner in financial success, we're here to support you with all your investment goals.`** The phrase `partner in financial success` was landing **twice in two adjacent blocks** (D-83). **We cut ours, never yours.** Measured on the built page: that phrase now appears **once**, in your copy.<br>**What we kept, and why.** `Secure your financial future` — your English page nowhere says it. `Support you in achieving your ambitious life projects` — `Our purpose` says `support you with all your investment goals`; **`ambitious life projects` is not `investment goals`**, and it is the one thing in this item your English page does not already say, so the item stays. `Build a long-term relationship based on trust and professionalism` — `Our purpose` mentions neither a long-term relationship nor trust. *(`professionalism` does also appear as the bare label `Our professionalism` in `Our unique approach`. We kept this item anyway: a one-word echo of a four-word label is not the same as repeating a sentence, and cutting it would lose `long-term relationship` and `trust` to save nothing.)*<br>**Unchanged from before:** `partenaire principal` was never upgraded to `leading` or `primary` while it shipped, and two French items end in an invisible no-break space the English does not carry — D-74 |
| NC-21 | English home page — a new `Our commitments` block | **⚠ THREE ITEMS AS SHIPPED, revised 2026-08-03 (D-100 / D-103).** `Our commitments` · `Stay informed in order to make well-informed decisions` · `Invest boldly without being imprudent` · `Keep all our commitments to ensure work well done and duty fulfilled`.<br>**Removed before shipping:** ~~`Act with integrity and communicate transparently`~~ — see the status column | `Nos engagements` · ~~`Agir de manière intègre et communiquer de façon transparente`~~ *(not shipped in English; **still live and unchanged on your French page**)* · `Demeurer informés pour prendre des décisions éclairées` · `Investir de manière audacieuse sans être imprudents` · `Respecter tous nos engagements pour assurer le travail bien fait et le devoir accompli` *(French home page, C-26)* | ⚠ needs explicit approval — **NBF sign-off required.**<br>**What we cut, and what it duplicated.** `Act with integrity and communicate transparently` is gone. It collided with **two** of your own live English strings at once: `Our values`, inside `Our purpose` above, reads **`We're committed to honesty, integrity and excellence.`**, and `Our unique approach` below lists **`Our integrity`** and **`Our transparency`**. That put `integrity` on the page **three times** (D-83) and `transparen-` twice. **All three statements were commitments to the same two things; two of the three were yours, so ours was the one that could go.** Measured on the built page: `integrity` now appears **twice, both in your own copy**, and `transparency` **once**.<br>**What we kept, and why.** `Stay informed…`, `Invest boldly…` and `Keep all our commitments…` — **your English page says none of these anywhere**, so each adds something rather than restating it. The list still opens under its own heading and still reads as a list of commitments.<br>**Unchanged from before:** `audacieuse` still renders as `boldly`, not `aggressively` or `fearlessly`; `imprudents` as `imprudent`, not `reckless` |
...
| NC-28 | `/en/our-firm/` — new section, above the National Bank material | Heading `Wolf Archard Group`, then:<br>`Our team brings together Laurent Achard and David Alexandre Wolf, wealth advisors in Montreal. Our mission is to secure your financial future, to support you in achieving your ambitious life projects and to be the main partner in your financial success. We are committed to acting with integrity, communicating transparently and building a long-term relationship based on trust and professionalism.` | **Heading** — your own English team name, **including the `Archard` misspelling**, because every other English string on the site carries it and spelling it differently here would quietly answer question 1 for you (D-76). **S1** — `Our team` is your own navigation label; `wealth advisors` (plural, lowercase) is a live string **on this same page**, in the `over 850` sentence just below; `Montreal` unaccented is this tree's own convention. **S2 / S3** — the `Our mission` and `Our commitments` items from NC-20 / NC-21. ⚠ **Note added 2026-08-03:** two of the clauses used here — `to be the main partner in your financial success` and `acting with integrity, communicating transparently` — were **removed from the home page** on that date (D-102 / D-103), so this paragraph is now the only English surface carrying them. **That is deliberate and this paragraph is unchanged.** They were cut on the home page because they collided with `Our purpose`, which sits beside them there; **`/en/our-firm/` has no `Our purpose` block and no `Our unique approach`, so nothing on this page repeats them.** Their authority does not depend on the home-page rows either way: each is a faithful rendering of a French clause live on your site — `Être le partenaire principal dans votre réussite financière` and `Agir de manière intègre et communiquer de façon transparente` (C-26) — which is what §6.5 requires. *(`F-51` separately flags that this page already makes three `partner` claims in a row; that row is unchanged, and cutting this sentence is one of the options it offers you.)* | ⚠ needs explicit approval — **and note it is three sentences where the French is four.** The French ends on your four-step process and **your English site has no four-step process** to end on — its four items are qualities, not steps. We wrote the shorter honest version rather than padding it. D-73 |
...
**Nothing was deduplicated or balanced between languages.** Where your English
page carries a report your French page does not (`Week at a Glance`), where the
same four guides are listed in a different order in each language, or where the
same report is credited to two different research groups — that is reproduced
as-is and written up in the C-series above.

⚠ **This paragraph used to begin "Nothing was deduplicated, reordered or
balanced".** **Two things have since been reordered, both on 2026-08-03, and
both are in the table below — `S-20` and `S-21`.** Nothing was deduplicated and
nothing was balanced between languages; those halves of the sentence still
stand, which is why it is amended rather than deleted. **Neither reorder changes
a word, a title, a link, an image or a description**, and each is reverted by
moving one object back. A blanket claim that quietly stopped being true is worse
than no claim, so it now names its own exceptions.
...
| S-20 | **ENGLISH news page only. `Week at a Glance` is filed in the wrong section, and the bank files it there.** It is a weekly markets report, and on the live page it sits at the **end of `Economic news`** — after both videos and both transcript panels — as a single card alone in a three-across grid. It is the only publication card on the page that is not in a publications section. Two sections further down, `Economic analysis` lists four cards of exactly the same kind, with the same `Read the latest version` button. | **Moved into `Economic analysis`, as the second card**, directly after `Weekly Economic Watch`. The two weekly reports now lead that section and the three monthlies follow. | A reader finishing a collapsed video transcript should not next meet a PDF card, still under a heading about videos. **Nothing was written and nothing was cut:** the title, the description, the button, the PDF address, the picture and its description are the same objects, character for character. The four cards already in `Economic analysis` keep their live order. **Your French page has no `Week at a Glance` and none was created.** Reverting is moving one object back — the code that rendered it in the old place is still there, unused, with a note saying so. |
| S-21 | **Both languages. `Actualités politiques de Trump` / `Trump's Policy Updates` is the FIRST thing under the page heading**, ahead of `Contenus en vedette` / `Spotlight: Now trending`. | **The two sections are swapped**, identically in both languages. Your news page now opens on National Bank's three featured articles; the policy block is second. **Nothing inside either section moved**, and the three policy documents keep their live order (the bank's own file numbering, 228 then 226, puts the newest first, which is right). | Three reasons, and the third is the one that decides it. **(1)** It is the first thing a prospective client reads on a regulated advisor's site, and it is a named foreign politician. **(2)** It is the only block on the page carrying neither a date nor a *"latest version"* link — everything else is either evergreen or dated. **(3) ⚠ It is the one block where your two languages tell different stories.** The English first card is titled `One Big Beautiful Bill Act \| Canadian Concerns Removed`; the French card for the same document pair is `One Big Beautiful Bill Act (OBBBA) - Projet d'article 899` and says nothing about the concerns being removed — see S-22. A block that reads as resolved in one language and unresolved in the other is the worst thing on the page to open with, and neither title may be touched. **No word, title, link, picture or description changed. Reverting is exchanging the two sections again. Yours to overturn.** |
| S-22 | **The same two documents are titled differently in your two languages, and only one language says the matter is closed.** English: `One Big Beautiful Bill Act \| Canadian Concerns Removed`. French: `One Big Beautiful Bill Act (OBBBA) - Projet d'article 899`. Both link the same pair of National Bank PDFs (`228-…-en` / `228-…-fr`). | **Both ship exactly as they are.** Retitling a bank publication link is not something we do — the label has to track the bank's. | This is the clearest evidence on the site that **National Bank updates one language and not the other**, and it is why S-21 moved that block out of the lead. A French reader is left with two cards whose titles both centre on `Projet d'article 899`, with nothing to say that the first one supersedes the second; an English reader is told plainly that the concerns were removed. **Ask NBF for the current French title.** Until then the French page reads as though the issue is still open. |
...
### D-80 … D-83 — reading the home and services pages as a visitor does

> Four items from a pass on 2026-08-03 that read `/`, `/en/`, `/services/` and
> `/en/services/` straight through, in both languages, **looking for places where
> a page does not hold together for the person reading it.** Not compliance; that
> is audited and closed.
>
> **`D-80` … `D-82` are applied and not one of them changes a character of copy.**
> Each is a layout rule that stopped matching the content it was written for —
> two of them because `F-21` moved a list and the stylesheet was never told.
> `D-83` is reported, because the only fixes for it are yours to authorise.
> **Nothing was written, no wording altered, no claim, figure or credential
> touched.** The four built pages were re-extracted after the change and the text
> is byte-identical to before it, in both languages.
>
> *(Numbered from `D-80` for the same reason `D-60`, `D-70` and `D-90` were: other
> people were editing this file at the same time, and a gap is cheaper than a
> collision.)*

| ID | Item | What we did, and why | Status |
|---|---|---|---|
| **D-80** | **Half of a band on your French home page was empty, and `F-21` is why.** | That band was built to hold **two** lists side by side — `Notre mission` and `Nos engagements` — because that is how your live page has them. `F-21` then moved `Nos engagements` down into `Un processus éprouvé`, to answer the colon your lead sentence ends on. **The stylesheet was never told.** It went on reserving the second column for a list that is no longer there, so from tablet width upward `Notre mission` sat at half width with **the right-hand half of a full-width band blank beside it** — the same failure the banners had before D-60, one section lower. The two-column rule is now conditional on there actually being two lists: **one list takes its own measure instead**, capped at the same 46rem your English `Ce qui nous distingue` is capped at, where that list also holds a column alone (D-72). **Your English home page still has both lists and still runs them as a pair — it is untouched.** No word changed. | Applied — pending sign-off |
| **D-81** | **`Nos engagements` was jammed against the sentence that introduces it, with no space at all.** | Also downstream of `F-21`. The list was mounted under the lead and given a name to style it by — and **nothing ever styled that name.** So the finished page read: your lead sentence, set large, ending on `…vous pourrez toujours compter sur:`, and then **at zero distance**, in small grey capitals, `NOS ENGAGEMENTS` — followed by a full band of air before the first process step. `F-21` made the colon resolve; the spacing made it read as a collision. The list now has room above it — **less than the process steps get below it, so it still reads as belonging to the sentence** rather than as a third thing wedged between two blocks — and the same 46rem measure it is held to everywhere else on the site. At the container's full width its hairlines ran twice as wide as any other instance of that list. No word changed. | Applied — pending sign-off |
| **D-82** | **`Philanthropy` was the one ragged block in the run of seven on your English services page.** | Six of the seven service blocks carry two lists, `Purpose` and `Main benefits`, and are laid out as two columns. **`Philanthropy` carries one** — `Setting up a foundation`, `Distribution of donations`, `Bequests in a will` — because that is all your live page gives it, and no label was invented to make it look even. But the two-column rule applied to it anyway, so its three items sat at half width with **an empty column beside them**, directly under six blocks that fill both. The two-column rule now applies only where there are two lists. **The single list takes the full column.** Nothing was added to it, and `Banking solutions` — which genuinely has one labelled list and one unlabelled one — still shows exactly that. No word changed. | Applied — pending sign-off |
| D-83 | ✅ **RESOLVED 2026-08-03 by `D-100` — you authorised the cut, and we cut ours, not yours.** *(Row preserved unedited below as the finding.)* ⚠ **Your English home page now says what it stands for three times running, and two of the three are things you just authorised.** | **Resolution first:** `Our purpose` **stays** — it is yours and it is live. Two sentences from **our** translation are removed instead: `Be the main partner in your financial success` (NC-20) and `Act with integrity and communicate transparently` (NC-21). Measured on the built page afterwards: `partner in financial success` **2 → 1**, `integrity` **3 → 2** *(both remaining are your own copy)*, `transparency` **2 → 1**. Nothing was rewritten and your French page is untouched. **`D-100` … `D-103` are the record.** *(The original finding follows.)* `Our purpose` is your live English copy: *"As your partner in financial success…"* and *"We're committed to honesty, integrity and excellence."* Immediately under it now sit `Our mission` and `Our commitments`, translated from your French page on 2026-08-03 (NC-20, NC-21) — and `Our mission` says *"Be the main partner in your financial success"* while `Our commitments` opens *"Act with integrity and communicate transparently"*. Then `Our unique approach`, one section further down, lists `Our integrity`. **`partner in financial success` lands twice in two adjacent blocks, and `integrity` three times on one page.** Nothing here is wrong and nothing is a claim — it is the same commitment stated three ways, in three registers, because **your French page and your English page each grew their own version of it and the translation has now put both on one page.** Your French home page does not have this, because it has no `Our purpose` block at all. **We changed nothing.** Every fix is a cut, and all three blocks are yours — two of them authorised by you the same day. **The cheapest version, if you want one: drop `Our purpose` from the English home page.** It is the one block in that section with no French counterpart, and `Our mission` and `Our commitments` now cover the same ground in your own words. That is your call, not ours. | Flagged, not changed |

---

### D-90 … D-97 — reading the team, profile and contact pages as a visitor does

> Eight items from a pass on 2026-08-03 that read those six pages straight
> through, in order, the way somebody arrives at them — **looking for places
> where what a page says and what it then shows do not match.** Not compliance;
> that is audited and closed.
>
> **Only `D-90` changed anything, and it removed a duplicate rather than adding
> a word.** The other seven are reported, because every fix they need is either
> yours to authorise or sits in a shared part of the site this pass was not
> permitted to edit. **No copy was written, no wording altered, and no
> credential touched, anywhere in this pass.**
>
> *(Numbered from `D-90` for the same reason `D-60` and `D-70` were: two other
> people were editing this file at the same time, and a gap is cheaper than a
> collision.)*

| ID | Item | What we did, and why | Status |
|---|---|---|---|
| **D-90** | **Each advisor's own page offered the same link twice, one screen apart.** | `Laurent Achard` and `David Alexandre Wolf` each have a page of their own. It holds a photograph, a name, a title and four rows — one screen, no scrolling on a laptop. It carried **`Notre équipe` above the name and `Notre équipe` again below the rows**: same words, same destination, both on screen together. The page's only way onward said *go back where you came from*, twice. **The lower one is gone.** The upper one stays, because it sits exactly where your live page's own `< Voir tous les conseillers` sits and it is the link D-22 already registered. `Notre équipe` is still in the top menu and still in the footer, so three routes back survive. Applied to all four pages — two advisors × two languages — because a tidy-up done in one language only is a new inconsistency. **Not one word changed.** | Applied — pending sign-off |
| D-91 | ✅ **INVESTIGATED AND DISPOSED 2026-08-03 — `D-101` (what changed) and `D-104` (what we found).** *(Row preserved unedited below as the finding.)* ⚠ **The four advisor pages do not tell a visitor anything the page they came from did not already.** | **Disposition first: the finding is confirmed, the routes stay, and the only thing removed is a promise we had added ourselves.** We checked National Bank's advisor record rather than assuming — **it holds nothing more** (`D-104`), and it turns out to hold an **empty biography field**, which is the answer to "what would make these pages worth having". We did **not** delete the four URLs. We removed the invisible whole-card overlay our build had laid across each team-page card, so only the **name** is a link, exactly as your live pages have it (`D-101`). *(The original finding follows.)* Read them side by side. A visitor on `Notre équipe` sees Laurent Achard's name, title, languages, phone and email on his card, clicks his name — and arrives at his name, title, languages, phone and email. The fourth row, the office address, **is printed in the footer of every page on the site**, including the one they just left. So of the four rows on the page, three repeat the card they clicked and the fourth repeats the footer. **The page is a click that costs a page load and returns nothing.** This is not a build defect — it is what National Bank's advisor record holds, and the record is all there is. **The fix is a few sentences about each man**, which only you can supply and which nobody may write for you. Until then the pages are honest but empty. If you would rather not have them at all, that is a URL and search-listing decision — say so and we will put the choice in front of you properly. | Flagged, not changed |
| D-92 | ⚠ **The same photograph of Laurent Achard is described two different ways on two of your pages.** | On `Notre équipe` his headshot is described `Laurent Achard, Conseiller en gestion de patrimoine`. On `Nous joindre` — **the identical picture file** — it is described `Photo de Laurent Achard, Conseiller en gestion de patrimoin , membre de l'équipe expert.`, the live wording with `patrimoine` cut off mid-word. Someone using a screen reader meets the same man twice and is told his title correctly once. The same split affects David Alexandre Wolf, and both men in English (`Photo of  Laurent Achard…`, double space and a trailing space). **Neither half is wrong on its own**: the team page uses the description we wrote for the rebuild (NC-07) and the contact page ships your live one exactly as it is, which is what F-01, F-07 and F-08 require while no editorial-fix approval exists. **The two rules meet on one photograph and disagree.** ⚠ **One word from you closes it in both directions at once**: approve the F-series and the live wording is corrected everywhere, or tell us to use the shorter description on both pages. We did not choose for you — either choice edits a description of a named, licensed advisor. | Flagged, not changed |
| D-93 | **Your office address, and the link to your opening hours, print twice on the contact page.** | The address block partway down `Nous joindre` gives `1 Place Ville-Marie, Bureau 1700, Montréal (Québec) H3B 2C1` and `Heures d'ouverture`. The footer of that same page, a screen lower, gives the identical address under the heading `Bureau` and the identical `Heures d'ouverture` link. **On the one page whose entire job is your coordinates, your coordinates appear twice.** We already do this correctly for the other half of the footer — D-11 switches off the footer's `Contactez-nous` band on this page precisely because its heading is this page's own `<h2>`. **The address column needs the same treatment and did not get it**, because switching it off means editing the shared footer, which this pass was not permitted to touch. **The fix is the same one line D-11 already established**, extended to the address column, on the two contact pages only. | Flagged, not changed — needs the shared footer, one line |
| D-94 | ⚠ **The bar pinned to the bottom of the screen always calls Laurent Achard — including on David Alexandre Wolf's own page.** | On a phone, every page carries a bar with two buttons, `Appeler` and `Écrire`. They are wired to the **first** advisor in the list, always. On `David Alexandre Wolf`'s own page, under his photograph and beside his own number, `Appeler` dials Laurent Achard and `Écrire` writes to Laurent Achard. The two men's names *are* attached for screen readers, but **a sighted visitor sees two unlabelled buttons and no reason to doubt them.** Confirmed on all four advisor pages and on both contact pages. This is in the shared bar, which this pass could not edit. **Three ways out, yours to pick:** put the name on the button, so it reads `Appeler Laurent Achard`; make the bar follow the page, so an advisor's own page offers his own number; or drop the two buttons and keep the panel, which already lists both men correctly. | Flagged, not changed — needs the shared contact bar |
| D-95 | ⚠ **A visitor meets your two advisors as five lines each, and your two collaborators as full profiles.** | On `Notre équipe`, under the heading `Notre équipe d'experts`, Laurent Achard and David Alexandre Wolf get a name, a title, a languages line, a number and an address — **thirteen words each, counted off the built page.** Under `Nos partenaires` immediately below, Lysane Tougas's `Biographie` runs to **94 words** and Jean-Francois Gobeil's to **149**, covering their degrees, their memberships and their experience. *(English: seven words each against 93 and 133 — the gap is wider there, because the English cards give one language line where the French give three.)* The page's one tinted band sits on the partners, so the section that stands out is also the long one. **Nothing on the page is wrong and the order is already right** — your own people come first. But the weight is inverted, and a visitor deciding whom to trust reads the two people they cannot contact and skims the two they can. **We changed nothing, because every available fix is either words you have not written or a design change made without being able to look at the result.** The real fix is a short biography for each advisor — you already have four on the site, so the shape is set. **A cheaper half-measure, if you want one: move the page's single tinted band from `Nos partenaires` up to `Notre équipe d'experts`**, so the emphasis lands on the two people a visitor is meant to phone. That is one word in one file, in each language, and no copy at all. | Flagged, not changed |
| D-96 | **Both advisors' cards list the same four facts in a different order on the team page than on the contact page.** | Team page: title, languages, phone, email. Contact page: title, phone, email, languages. **Your live site puts languages last on both pages**, so the contact page is the faithful one and the team page is where the rebuild drifted — the order is fixed inside the shared card component rather than set by either page. Small, but a visitor moving between the two pages meets the same card with its lines shuffled. **One line in the shared component restores your live order on both.** | Flagged, not changed — needs the shared card component |
| D-97 | **`Nous joindre` is the contact page's own `<h1>` and also the heading of the panel behind the pinned bar.** | Both are on the page at once, at two different levels, naming two different things — the page, and a slide-up panel of phone numbers. The same holds in English, where `Contact us` is both. Nothing a sighted visitor trips over; **for someone navigating by headings it is two entries with one name.** The panel's heading lives in the shared contact bar. **If you want it distinct, the wording is yours to choose** — we will not name a part of your site for you. | Flagged, not changed — needs the shared contact bar |

---

### D-100 … D-104 — the three recommendations you authorised on 2026-08-03

> **These are the three items you signed off, and what each one actually cost.**
> `D-83` and `D-91` were flagged as *"yours to authorise"*; you authorised them,
> plus `F-30`. All three are now applied.
>
> **The rule we worked to, on all three: cut from our own work, never from
> yours.** Where a collision involved one of your live strings and one of ours,
> **ours went.** Where it involved only your strings, nothing went. Where an
> affordance over-promised, we shrank ours to the size of your live page's.
>
> **Not one sentence was written.** Every change is a deletion or a removal.
> Across the whole build, exactly **two of seventeen pages** changed a single
> character of text: your English home page (**−93 characters**, being the two
> sentences named in `D-102` and `D-103`, and nothing else) and your French
> services page (**−24 characters**, being the label prefix in `F-60`, and
> nothing else). The other fifteen are byte-identical, and **no heading anywhere
> on the site changed at all**.
>
> *(Numbered from `D-100` for the same reason `D-60`, `D-70`, `D-80` and `D-90`
> were: a gap is cheaper than a collision.)*

| ID | Item | What we did, and why | Status |
|---|---|---|---|
| **D-100** | **Your English home page said the same thing three times. It now says it once — and the one that survived is yours.** | This is the fix for `D-83`. `Our purpose` — `Our objectives` and `Our values` — is your **live English copy** and it **stays, untouched**. What we removed is two items from the translations you authorised the same morning, because those two were the ones repeating it. **Two sentences, both ours, both cut whole rather than reworded** — `D-102` and `D-103` quote each one and name exactly what it duplicated. `Our mission` and `Our commitments` now carry **three items each instead of four**; both blocks still stand, because the remaining six each say something your English page does not. Measured on the built page, before → after: `partner in financial success` **2 → 1**, `integrity` **3 → 2**, `transparency` **2 → 1** — and every remaining instance of all three is **your own live wording**. **Your French home page is untouched and still carries all eight items** (verified item by item in the built French page). **Reversible by restoring two strings in one file.** | **Applied — client decision 2026-08-03, pending NBF sign-off** |
| **D-101** | **Every advisor card on your team page was one big click target for a page that returns nothing. Now only the name is.** | This is the fix for `D-91`, and it is the *only* thing we changed there. Our build laid an invisible overlay across each card, so a click on the photograph, the job title or the languages line navigated to that advisor's profile page — **a page that repeats the card the visitor just clicked** (`D-91`, confirmed, see `D-104`). **Your live pages have no such overlay**: `notre-equipe.html` and `our-team.html` both link the **name only**, inside the heading. The overlay was ours, so the overlay went. **The link is completely unchanged** — same name, same wording, same destination, on both team pages in both languages. **No route was deleted**: all four profile pages still build, still carry their four rows, still link back to the team page, still pair FR↔EN, and **all four are still in the sitemap** — verified in the built output. Your contact page, which also links these profiles on the live site, is untouched. *(Our own contact-page card already refused the whole-card pattern and explains why; the team page is now consistent with it.)* **Reversible by restoring two CSS rules in one component.** | **Applied — client decision 2026-08-03.** No word changed, no URL changed |
| **D-102** | **Cut from `Our mission`: `Be the main partner in your financial success`** | **What it duplicated:** your own `Our objectives`, one section above it, opens **`As your partner in financial success, we're here to support you with all your investment goals.`** Same claim, same phrase, two adjacent blocks. **We cut ours.** The French item it rendered — `Être le partenaire principal dans votre réussite financière` — **is still live and unchanged on your French home page**, and the English clause still appears once on `/en/our-firm/`, where nothing collides with it (`NC-28`). **It was cut, not softened**: rewording it into something like *"a partner you can rely on"* would have been writing new English about you, which is exactly what the amendment forbids. | **Applied — client decision 2026-08-03** |
| **D-103** | **Cut from `Our commitments`: `Act with integrity and communicate transparently`** | **What it duplicated — twice over.** `Our values`, inside `Our purpose` above, reads **`We're committed to honesty, integrity and excellence.`** And `Our unique approach`, below, lists **`Our integrity`** and **`Our transparency`** among its four items. Three statements of the same two commitments; **two of the three are your live copy, so ours is the one that could go.** The French item — `Agir de manière intègre et communiquer de façon transparente` — **is still live and unchanged on your French home page**, and the English clause still appears once on `/en/our-firm/` (`NC-28`), which has neither `Our purpose` nor `Our unique approach`. The three remaining commitments — `Stay informed…`, `Invest boldly…`, `Keep all our commitments…` — say things **your English page says nowhere else**, which is why they stayed. | **Applied — client decision 2026-08-03** |
| **D-104** | ⚠ **We checked what National Bank actually holds about your two advisors. It holds a biography field, and yours are blank.** | **The question `D-91` left open was whether NBF's advisor record carries anything the pages do not show.** On a previous client's site it carried designations that appeared nowhere public, so we did not assume. **We looked, twice.** (1) Your captured pages: all four render National Bank's own biography component, `advisorBiographyUI`, and it outputs **nothing at all** — an empty container, on every page, in both languages. (2) We re-fetched all four live pages on 2026-08-03 as a second check: **no designation, no fax, no LinkedIn, no education, no years of experience, no biography.** The only extra values in the record are National Bank's internal tags for **languages** and **office**, and your pages already show both. **So the pages are thin because the record is blank, not because the platform is limited — and the slot for the missing part is already built and already on your live site.** **What you would need to supply:** one short biography per advisor, in French and English — four paragraphs in total. You already have four of them on your team page for Lysane Tougas and Jean-Francois Gobeil, so the shape and the length are set by your own site. **We will not write them**, and neither will NBF: they are statements about two named, licensed individuals, and every sentence would need compliance sign-off exactly as `NC-25` and `NC-26` do. **The payoff is not only here** — filling that field fills it on `fbngp.ca` and `nbfwm.ca` too, because it is National Bank's field, not ours. Until then the four pages stay exactly as they are: honest, reachable at their own addresses, and no longer promised as more than they are (`D-101`). | **Reported — the ask is now specific, and it is one paragraph per advisor per language** |
...
| 2026-08-03 | **Two things on the news page were moved, and nothing on it was rewritten** | Read end to end as a visitor, the syndicated page had two ordering faults. **(1) `S-20`** — on the English page `Week at a Glance`, a weekly markets report, sat at the end of `Economic news` after both videos and both transcripts, alone in a three-across grid, the only publication card on the page outside a publications section. It is now the second card in `Economic analysis`, beside the other weekly. **(2) `S-21`** — both pages opened on `Actualités politiques de Trump` / `Trump's Policy Updates`: a named foreign politician as the first thing a prospective client reads, in the only block on the page carrying no date, and — the deciding fact — **the only block whose two languages disagree**, since the English first card says `Canadian Concerns Removed` and the French one does not (`S-22`). It is now second, behind National Bank's featured articles. **No word, title, link, picture or description changed in either move; the external link set is byte-identical before and after — 135 distinct addresses, 575 occurrences, verified page by page.** Both are reverted by moving one object back. **Yours to overturn.** |
| 2026-08-03 | **Four things on `Notre firme` / `Our Firm` read wrong and none of them could be fixed without writing a sentence** | So none were. `F-50` — your new paragraph's *mission* and National Bank's *mission* are stated one after the other for what a reader takes to be the same firm. `F-51` — the English page makes three `partner` claims in three consecutive blocks; the French page makes one. `F-52` — `nous` / `we` starts as your two advisors and becomes a 850-advisor network two paragraphs later, with the same job title in both places; **this one did not exist before `NC-27` / `NC-28` were added, and it is the only place on the site where our own addition changed how the bank's copy reads.** `F-53` — the `Contactez-nous` / `Contact us` heading is one level too deep, so a screen reader announces both direct phone numbers as part of *About National Bank of Canada*; it is a one-character fix in a shared component that page work is not allowed to touch. All four are written up for you to decide. |
| 2026-08-03 | ✅ **Three open recommendations authorised and applied — and all three were cuts** | You approved `F-30`, `D-83` and `D-91`. **(1) `F-30` → `F-60`:** the 24 characters `Planification fiscale : ` are deleted from the front of the French tax-planning paragraph, so it no longer repeats the heading above it. Recorded as **the first amendment to §6**, deliberately scoped to one row (`content/COMPLIANCE.md` **§6.6**) — **`F-03`, in the same sentence, is still PENDING and still ships sic.** **(2) `D-83` → `D-100`:** two sentences of **our own** translation left the English home page because they repeated your live `Our purpose` block; `Our mission` and `Our commitments` now carry three items each. **Your copy was not touched and your French page still carries all eight.** **(3) `D-91` → `D-101` / `D-104`:** National Bank's advisor record was checked rather than assumed — it holds an **empty biography field** and nothing else we are not already showing — so the four routes stay and the invisible whole-card overlay **we** had added to the team-page cards is removed, leaving the name as the link, exactly as your live pages have it. **Nothing was written; every change is a deletion.** Verified: `npm run build` **17 pages**, `npx astro check` **0 errors / 0 warnings / 0 hints**, **0 broken internal links, 0 missing assets, 0 heading-level skips, 0 cross-language leaks**, both team pages still render **5 legal notes with both markers resolving**, `David Alexandre Wolf` unhyphenated everywhere, and the external-link set **identical before and after — 135 distinct addresses, 575 occurrences, zero added, zero removed, zero changed in count.** Text changed on exactly **2 of 17 pages** (−93 characters on `/en/`, −24 on `/services/`) and **no heading changed anywhere.** A sweep of **all 240 headings on all 17 pages** found the heading-repeated-as-label pattern **once before the fix and zero after**, and the detector was proved against a re-inserted copy rather than trusted. |
| 2026-08-03 | **The sticky contact bar dialled the wrong advisor** | The bar's two actions always used the first-listed advisor (`ContactStrip.astro`: `const primary = people[0]`), so on **David Alexandre Wolf's own profile page** the call and mail buttons reached **Laurent Achard** — in both languages. The bar's names are screen-reader-only, so a sighted visitor saw two unlabelled buttons with no way to tell. `BaseLayout` gained `primaryContactEmail`; the advisor profile pages pass their own advisor, and the bar dials that person. Every other page keeps the previous first-listed behaviour. Registered `D-98`. A defect we introduced, not one inherited from your site. |
...
- [ ] ⚠ **One short biography per advisor, in French and English — four paragraphs** (D-104). National Bank's advisor record already has the field and both advisors' are **blank**, which is the whole reason the four profile pages repeat the cards that link to them. **Only Laurent Achard and David Alexandre Wolf can supply them**, and each will need NBF compliance sign-off like NC-25 / NC-26. Filling the field fills it on your live `fbngp.ca` / `nbfwm.ca` pages too
- [ ] F-03 — one letter, in the same sentence F-60 already edits (`à l'intérieure` → `à l'intérieur`). Approving it costs one more word from you and no more work than F-60 did
...
- [ ] ⚠ **S-21 — your news page no longer opens on the Trump policy block.** Confirm you want it second rather than first *(reverted by swapping two objects back)*
- [ ] S-20 — `Week at a Glance` moved into `Economic analysis` on the English news page
- [ ] S-22 — ask NBF for the current **French** title of the OBBBA document; the English one says the Canadian concerns were removed and the French one does not
- [ ] F-50 / F-52 — the two *mission* statements, and who `nous` / `we` means, on `Notre firme` / `Our Firm`
- [ ] F-53 — `Contact us` heading level, and the duplicated `Contact us` heading on every English page *(shared components — one owner, one decision)*
