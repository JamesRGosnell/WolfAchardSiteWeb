...
| F-20 | Accueil (FR) | `Nos conseils, notre accompagnement et notre dévouement sont très appréciés de tous nos clients. En effet, vous pourrez toujours compter sur:` | `Nos conseils, notre accompagnement et notre dévouement sont très appréciés de tous nos clients.` | **Applied — pending sign-off.** Deletion only; no word rewritten, none invented. The live colon promises a list that does not follow: the live document order is `<h2> Un processus éprouvé` → this `<p>` → `<h3> Faisons connaissance`, a process step rather than something a client can count on. Verified in `source-assets/html/fr-home.html`. **The client's own `Nos engagements` list is four things a client can count on** — if they prefer the colon kept, that list is the honest continuation and we would move it here. English is unaffected: its process block has no lead-in. |
