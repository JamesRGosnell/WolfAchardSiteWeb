...
| F-52 | **Notre firme / Our Firm**, **both languages** — who `nous` / `we` means, and when it changes | Your paragraph opens `Notre équipe réunit Laurent Achard et David Alexandre Wolf, conseillers en gestion de patrimoine à Montréal` / `Our team brings together … wealth advisors in Montreal`. Two paragraphs later, in National Bank's copy: `**Notre** réseau compte près de 850 **conseillers en gestion de patrimoine** dans 100 succursales` / `**Our** network includes over 850 **wealth advisors** at 100 branches`. **Same pronoun, same job title, and the referent has silently gone from two people to a national network.** | **unchanged** | Shipped as built — **reported, not repaired.** ⚠ **This is a side-effect of D-15, and it did not exist before your paragraph was added.** When the page was 100% National Bank copy under a heading naming FBNGP, every `nous` on it meant FBNGP and there was nothing to confuse. Putting your team first — which is right, and which you asked for — makes yours the first `nous` a reader meets, and they carry it downward. **Nothing on the page claims your team has 850 advisors**, and the sentence does name the network, so we judge the risk low. But it is the one place on the site where our own addition changed how the bank's copy reads, and you should see it before launch rather than after. |
...
| F-52 | Notre firme / Our firm | Our own `NC-27`/`NC-28` paragraph read `…Laurent Achard et David Alexandre Wolf, **conseillers en gestion de patrimoine** à Montréal.` / `…**wealth advisors** in Montreal.` — a plural borrowed from the `près de 850 conseillers en gestion de patrimoine` / `over 850 wealth advisors` sentence two paragraphs below | `…Laurent Achard et David Alexandre Wolf, à Montréal.` / `…in Montreal.` | **Applied — cut from our own authored copy, not the client's.** The identical phrase meant two people and then 850, so `nous`/`we` silently changed referent mid-page. **This defect did not exist before we added NC-27/NC-28** — it is the one place our addition changed how National Bank's copy reads. Verified: each phrase now appears once per page. Their titles are unaffected and still appear on the team page and both profile pages. |
...
| D-98 | Sticky contact bar, all 17 pages | The bar's two actions always used the first-listed advisor (`ContactStrip.astro`: `const primary = people[0]`), so on **David Alexandre Wolf's own profile page** the call and mail buttons reached **Laurent Achard** — in both languages. The bar's names are screen-reader-only, so a sighted visitor saw two unlabelled buttons with no way to tell. | `BaseLayout` gained `primaryContactEmail`; the advisor profile pages pass their own advisor, and the bar dials that person. Every other page keeps the previous first-listed behaviour. | **Applied — build defect we introduced, not inherited.** No copy changed. Verified in `dist/`: all four profile pages now resolve to their own advisor's address. |
